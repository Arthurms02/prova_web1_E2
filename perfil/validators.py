...
def validar_email_institucional_aluno(email_institucional):
    """
    Valida se o email institucional pertence ao domínio correto para alunos
    """
    
    pattern = r'^[a-zA-Z0-9]+([._]?[a-zA-Z0-9]+)*@(aluno)\.(unifip\.br|uepb\.br|ufcg\.br)$'
    if not re.match(pattern, email_institucional):
        return False
    return True